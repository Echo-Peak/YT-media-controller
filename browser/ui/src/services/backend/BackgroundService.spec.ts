...
beforeEach(() => {
  resetWSMock();
  jest.clearAllMocks();
  delete process.env.REACT_APP_API_SERVER_PORT;
});
...
describe('BackendService (original API)', () => {
  ...
  test('init falls back to REACT_APP_API_SERVER_PORT', async () => {
    getChromeStorageKeys.mockResolvedValue({});
    process.env.REACT_APP_API_SERVER_PORT = '7777';
